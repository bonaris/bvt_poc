...
def validate_plp_header(driver):
    plp_page = PlpPage(driver)
    Validator.validate_header(driver=driver, actual=plp_page.get_header_text(), expected=plp_page.expected_header)


@then('Product Grid is displayed on the page')
def validate_plp_grid(driver):
    plp_page = PlpPage(driver)
